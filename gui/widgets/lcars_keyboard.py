...
###############################################################################
# LCARS rehash of the Kivy Vkeyboard, changes noted with comments in this style
###############################################################################
...
class LCARSKeyboard(Scatter):
    ...
    key_margin = ListProperty([2.5, 2.5, 2.5, 2.5])
    ...
    '''
    ##############################################################
    # Added to allow specific font colors
    ...
    ##############################################################
    # Modifed to allow different key type colors
    key_background_color_letter = ColorProperty(rgba('#7788FF'))
    key_background_color_control = ColorProperty(rgba('#CC88FF'))
    key_background_color_symbol = ColorProperty(rgba('#FFCC33'))
    key_background_color_number = ColorProperty(rgba('88CCFF'))
    ...
    :attr:`key_background_color_letter` is a :class:`~kivy.properties.ColorProperty`
    ...
    key_border = ListProperty([0, 0, 0, 0])
    ...
    def __init__(self, **kwargs):
        ...
        fbind('layout', layout)
        super(LCARSKeyboard, self).__init__(**kwargs)
...
###############################################################################
# Modified to allow different key colors depending on function
        with self.active_keys_layer:
            for line_nb, index in active_keys.values():
                ...
                if len(text) > 1:
                    Color(*self.key_background_color_control)  # Control Key
                elif text.isdigit():
                    Color(*self.key_background_color_number)  # Numeric Key
                elif text.isalpha():
                    Color(*self.key_background_color_letter)  # Letter Key
                else:
                    Color(*self.key_background_color_symbol)  # Symbol Key
            ...
            for line_nb in range(1, layout_rows + 1):
                ...
                for pos, size in layout_geometry['LINE_%d' % line_nb]:
                    ...
                    if len(text) > 1:
                        ...
                        Color(*self.key_background_color_symbol)  # Symbol Key
                    BorderImage(texture=texture, pos=pos,
                                size=size, border=self.key_border)

###############################################################################
# Modified to allow font color to be changed
        # then draw the text
        for line_nb in range(1, layout_rows + 1):
            key_nb = 0
            for pos, size in layout_geometry['LINE_%d' % line_nb]:
    ...
    def on_touch_down(self, touch):
        ...
        if not self.collide_margin(x, y):
            ...
            super(LCARSKeyboard, self).on_touch_down(touch)
        return True
    ...
    def on_touch_up(self, touch):
        ...
        return super(LCARSKeyboard, self).on_touch_up(touch)
